import "server-only";
import { providerKey, credentialScope } from "./credentials";
...
export async function konbiniRequest(path: string, params: Record<string, string> = {}, signal = AbortSignal.timeout(55_000)): Promise<Record<string, unknown>> {
  ...
  if (!token || /\s/.test(token)) throw new KonbiniError(503, credentialScope() === "invalid-session" ? "key_session_invalid" : "not_configured");
  try {

  ...
  return <div className={`profile-explorer__grid${people ? " profile-explorer__grid--people" : ""}`}>
    {items.map(item => <article className={`profile-explorer__card${people ? " profile-explorer__card--person" : ""}`} key={item.id}>
      ...
      <div className="profile-explorer__card-body">
        ...
        {!people && (item.video || ((section === "posts" || section === "reposts") && /^\d+$/.test(item.id))) && <button type="button" className="profile-explorer__action" aria-pressed={playing === item.id} onClick={() => setPlaying(playing === item.id ? undefined : item.id)}>{playing === item.id ? copy.profile.closePlayer : copy.profile.watch}</button>}
      </div>

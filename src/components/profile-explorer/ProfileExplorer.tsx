...
export function ProfileExplorer({ username }: { username: string }) {
  ...
  return <section className="profile-explorer" aria-labelledby="profile-heading">
    ...
    <div className="profile-explorer__tabs" role="group" aria-label={copy.profile.heading}>
      <a className="profile-explorer__tab" href="#story-player">{copy.profile.stories}</a>
      {profileSections.map(key => <button type="button" className={`profile-explorer__tab${key === section ? " profile-explorer__tab--active" : ""}`} aria-pressed={key === section} key={key} onClick={() => void load(key)}>{copy.profile.sections[key]}</button>)}
    </div>
